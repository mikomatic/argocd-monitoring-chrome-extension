...
const ArgoApplicationRow = ({ currentEnv, currentApp }: Props) => {
    ...
    if (status === 'Degraded') {
      return (
        <span className="icon" style={{ color: 'red' }}>
          <i className="material-icons">heart_broken</i>
          {status}
        </span>
      );
    } else if (status === 'Missing') {
      return (
        <span className="icon" style={{ color: '#ced4da' }}>
      ...
      return (
        <span className="icon" style={{ color: '#12b886' }}>
      ...
      return (
        <span className="icon" style={{ color: '#868e96' }}>
  ...
  return (
    <a
      ...
      target="_blank"
      key={currentApp.metadata.name}
      className="panel-block is-active"
    >
      <div className="columns is-mobile" style={{ width: '100%' }}>
        <div className="column is-half">{currentApp.metadata.name}</div>
